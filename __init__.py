...
class MyProperties(bpy.types.PropertyGroup):
    ...
    train_speed: bpy.props.FloatProperty(
        name="Train Duration", soft_min=20, soft_max=50, default=20)

    refresh_timer: bpy.props.FloatProperty(
        name="Refresh Timer", soft_min=1, soft_max=10, default=1)

    pixel_level: bpy.props.FloatProperty(
        name="Image Pixel Level", soft_min=0.01, soft_max=11, default=0.01)

    panel_change_timer: bpy.props.FloatProperty(
...
class SPOTIFY_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        token_url = box.operator(
            'wm.url_open', text="Get Spotify Token", icon="URL")
...
class Songcover():
    def __init__(self):
        ...
        Songcover.create_board_from_image(
            Songcover.getLinkToCurUserImage(), "profile", PROFILE_POSITION)
    ...
    def getCurUserTopArtists():
        ...
        if (r.status_code == 200):
            ...
            if (len(respJson["items"]) < 3):
                topArtists = ["Artists of the Month", "No Data",
                              "Artists of the Month", "No Data"]
                return topArtists
            else:
                topArtists = ["Artists of the Month", respJson["items"][0]["name"],
                              respJson["items"][1]["name"], respJson["items"][2]["name"]]
                return topArtists
        else:
            topArtists = ["Artists of the Month", "No Data",
                          "Artists of the Month", "No Data"]
    ...
    def create_writing(name, planeName, text, position, rotation, scale):
        ...
        titel_obj.rotation_euler = rotation
        titel_obj.scale = scale
        tram = bpy.data.objects[planeName]
        titel_obj.parent = tram
        titel_obj.location = position
    ...
    def create_environment():
        ...
        Songcover.create_writing("Song Titel", "Strassenbahn", Songcover.getCurrentlyPlayedSong()["name"],
                              (+4, +0.35, -0.78), (pi/2, 0, pi), (0.4, 0.4, 0.4))
        Songcover.create_writing("Displayname", "Halterung_4", Songcover.getCurUserDisplayName(),
                              (-0.34, -0.85, -0.9), (pi/2, -(pi/2), pi), (0.26, 1.45, 1))
        Songcover.create_writing("Top-Artists", "Halterung_5", Songcover.getCurUserTopArtists()[0], (1.32, -0.88, -0.2),
                              (pi/2, 0, pi), (0.32, 0.61, 0.65))
        Songcover.create_writing("Top-Track", "Halterung_3", Songcover.getCurUserTopSong()[1],
                              (0.38, -0.75, 0.94), (pi/2, (pi/2), pi), (0.22, 1.34, 1))
    ...
    def train_animation(last_frame):
        ...
        train_speed = math.floor(
            bpy.data.scenes["Scene"].my_tool.train_speed)  # min. 20 - max. 50
        if train_speed < 20:
            train_speed = 20
